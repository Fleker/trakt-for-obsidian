...
const got = async (req) => {
    ...
    // Use Obsidian's requestUrl (set in main.ts) to bypass CORS.
    // fetch() is blocked by Obsidian's renderer because api.trakt.tv
    // doesn't include Access-Control-Allow-Origin: app://obsidian.md.
    if (globalThis.__obsidianRequestUrl) {
        console.debug(
            url,
            req.method,
            req.headers,
        ...
        const response = await globalThis.__obsidianRequestUrl({
            url,
...
const repoUrl = 'https://github.com/vankasteelj/trakt.tv'
const defaultUa = `${pkg.name}/${pkg.version} (NodeJS; +${repoUrl})`;
